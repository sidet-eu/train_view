...
const carrierLogos = {
    ...
    'Przewozy Regionalne Sp. z o.o.': 'clogo/polregio.png',
    'MÁV-Start Zrt.': 'clogo/mavstart.png',
...
function createTrainDiv(train, showStar = true) {
    ...
    const hasMultipleWords = train.train_name && train.train_name.trim().includes(' ');
    ...
    var trainNumberEl = $('<p></p>')
        ...
        .html(`
            ...
            ${train.type} ${train.train_number}${hasMultipleWords ? '<br>' : ' '}${train.train_name}
        `);
    ...
    trainNumberEl.css('position', 'relative');
    trainNumberEl.append(buttonsContainer);
    ...
    buttonsContainer.append(detailsButton);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            data.forEach(function(train) {
                var trainDiv = createTrainDiv(train);
...
$(document).ready(function() {
    Object.values(carrierLogos).forEach(logoPath => {
        const img = new Image();
        img.src = logoPath;
    });
